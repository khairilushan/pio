...
- Trace important values producer -> transformer -> consumer -> terminal effect and inspect call sites and framework wiring.
- Check correctness, nullability, bounds, parsing, compatibility, failure behavior, concurrency, cancellation, cleanup, security, privacy, permissions, logging, accessibility, and UI states where relevant.
- Prefer simple ownership-aligned code; report abstractions only for concrete maintenance risk.
- Inspect realistic regression coverage without running it.
- A must-fix violates the requested outcome or repository requirement, risks compilation/runtime/integration/security/privacy/data/accessibility failure, or omits required regression coverage for concretely changed behavior.
- Suggestions are optional improvements. Questions are only confidence-blocking evidence gaps.
- Cite path and line when available. Report no style preferences.
`;

export function reviewPrompt(task: string, baseline: string, focus: string): string {
	return `Review the current workspace changes with focus on ${focus}.\n\nTask contract:\n${task}\n\nPre-existing user-owned baseline:\n${baseline}\n${REVIEW_STANDARD}\nReturn valid JSON only:\n{\n  "mustFixes": [{"key":"stable semantic key without line numbers","title":"...","path":"...","line":1,"impact":"...","direction":"..."}],\n  "suggestions": [{"title":"...","path":"...","reason":"..."}],\n  "questions": ["..."],\n  "summary": "..."\n}\nLimits: 10 must-fixes, 6 suggestions, 6 questions.`;
}

export function triagePrompt(task: string, findings: ReviewFinding[]): string {
	return `Verify, merge, and deduplicate these proposed must-fixes against the current code and task contract. Keep only actionable must-fixes supported by repository evidence. Preserve stable semantic keys.\n\nTask:\n${task}\n\nFindings:\n${JSON.stringify(findings, null, 2)}\n\nReturn valid JSON only in the normal review shape with verified findings in mustFixes, empty suggestions unless essential, confidence-blocking questions only, and a concise summary.`;
}

export function fixerPrompt(task: string, plan: ApprovedPlan, findings: ReviewFinding[], baseline: string, round: number): string {
	return `Fix only these verified must-fixes in review round ${round}.\n\nTask contract:\n${task}\n\nApproved plan:\n${JSON.stringify(plan, null, 2)}\n\nMust-fixes:\n${JSON.stringify(findings, null, 2)}\n\nPre-existing user-owned baseline:\n${baseline}\n\nReturn valid JSON only using the implementation receipt shape. Set workItem to "review-fix-${round}". Include at least one validation entry; if no check was allowed or run, use outcome "skipped" and state the exact reason.`;
}

export function verifierPrompt(task: string, findings: ReviewFinding[], round: number): string {
	return `Verify review-fix round ${round}. Confirm each exact semantic key, inspect adjacent regressions introduced by the fixes, and report only unresolved or newly introduced must-fixes. Do not re-review the entire feature.\n\nTask:\n${task}\n\nFinding keys and contracts:\n${JSON.stringify(findings, null, 2)}\n\nReturn valid JSON only in the normal review shape.`;
}
